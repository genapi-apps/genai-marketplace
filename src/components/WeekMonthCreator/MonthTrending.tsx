...
const MonthTrending: FC<CardCategory5Props>  = ({data}) => {
   
  return (
    <>

    <h2 className='text-xs text-gray-600 font-semibold  mb-3'>Top Model this Month</h2>
       <div className='flex flex-col gap-5'>
   
      {data && data.length>0 ? data.slice(0, 5).map((item:any,i:any)=>{
        return      <div className={`nc-CardNFT relative flex gap-2 group `} key={i}>
      <div className="relative flex-shrink-0 ">
        <div className="flex w-36 h-[75px] rounded  overflow-hidden z-0">
          ...
          return ite.type === "thumbnail" &&       <NcImage
            containerClassName="flex w-36 h-18 rounded overflow-hidden z-0"
